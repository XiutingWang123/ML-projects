# -*- coding: utf-8 -*-
"""
Created on Tue Oct 11 22:41:56 2016
...
from crossValidation import LeaveOneOutCV


USAGE = "<train-set-file> <test-set-file> k1 k2 k3"
...
def main():
	trainSetFile, testSetFile, k1, k2, k3 = getArgs()
	...
	for k in [k1, k2, k3]:
		...
		totalError.append((k, error))
	
	bestK = 0
	minError = sys.maxsize
	for item in totalError:
		if classification:
			print('Number of incorrectly classified instances for k = {0} : {1}'.format(item[0], item[1]))	
		else:
			print('Mean absolute error for k = {0} : {1}'.format(item[0], item[1]))
		if item[1] < minError:
			minError = item[1]
			bestK = item[0]
		
	knn = KNeighborsClassifier(trainData, testData, bestK, classification, labels)
	predictedData = knn.getPrediction();
	
	print('Best k value : {0}'.format(bestK))	
	if classification:
		numCorrect = 0
		acc = 0
